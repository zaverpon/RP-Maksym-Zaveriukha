...
static void send_buffer_lock_checked(void)
{
    ...
    if (rc != 0) {
        fprintf(stderr, "pthread_mutex_lock(g_send_buffer_lock) failed: %d\n", rc);
        exit(1);
    }
    if (g_send_buffer_active) {
        fprintf(stderr, "send buffer is already active\n");
        exit(1);
    }
...
static void send_buffer_unlock_checked(void)
{
    int rc;
    if (!g_send_buffer_active) {
        fprintf(stderr, "send buffer is not active\n");
        exit(1);
    }
    ...
    if (rc != 0) {
        fprintf(stderr, "pthread_mutex_unlock(g_send_buffer_lock) failed: %d\n", rc);
        exit(1);
    }
}
